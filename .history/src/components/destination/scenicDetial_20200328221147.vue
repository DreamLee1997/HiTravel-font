 ...
 * @LastEditors: lixiang
 * @LastEditTime: 2020-03-28 22:11:47
 -->
<template>
  <div class="hotelDetial">
    <div class="main" v-loading="scenicLoading&&listLoading">
      <div class="main-in">
        ...
        <div class="main-in-bottom">
          <div class="table-container">
            <el-table ref="ticketTable" :data="ticketList" style="width: 100%">
    ...
    <el-dialog title="确认订单信息" :visible.sync="editDialogVisible" width="616px">
      <el-form v-loading="isloading" :model="orderInfo" size="small" label-width="120px">
        ...
        <el-form-item label="门票名称：">
          <span style>{{orderInfo.ticketName}}</span>
        </el-form-item>
    ...
    <el-dialog title="" :visible.sync="dialogVisible" width="316px">
      是否继续预定门票？
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false" size="mini">继续预定</el-button>
        <el-button type="primary" @click="handleToOrder" size="mini">查看订单</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeDate(e) {
      ...
      this.orderInfo.total = this.orderInfo.sellPrice * this.orderInfo.num;
      this.$forceUpdate();
    },
    ...
    handleEditConfirm() {
      ...
      ids.push(self.orderInfo.ticketId);
      nums.push(self.orderInfo.num);
      types.push(1);
      dates.push(self.orderInfo.ticketTime);
      let params = {
        dates: dates,
        ids: ids,
        nums: nums,
