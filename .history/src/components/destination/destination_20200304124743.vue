...
export default {
    data () {
        return {
            destination:"",
            areaCode:'',
            cityCodeArray:['350200', '310100', '330900', '320500', '440300'],
            props: {
                lazy: true,
    ...
    methods:{
        ...
        toDestination(val){
            ...
            self.handleSearch(self.cityCodeArray[val])
        },
        //搜索
        handleSearch(cityCodeVal){
            let self = this;
            let params = self.listQuery;
            if(cityCodeVal){
                params.cityCode = cityCodeVal;
                self.$router.push({ path: "/destination/scenicRmd", query: { cityCode: cityCodeVal }});
            }else{
                var pathvalue = this.$refs.cascaderAddr.getCheckedNodes()[0];
                params.cityCode = pathvalue.path[1];
                self.$router.push({ path: "/destination/scenicRmd", query: { cityCode: params.cityCode }});
            }
            self.isShow = false;
            fetchList(
                params.page,
