    ...
    <div class="banner">
      <div class="banner-in">
        <div class="pian">
          ...
          <div class="luxian">
            该线路基于
            <span>43</span>位蜂蜂游记生成，
            <span>22%</span>的蜂蜂都会选择此路线
          </div>
          <div class="jiantou">
            <a href>丽江古城</a>→
            <a href>丽江四方街</a>
        ...
        <div class="search">
          ...
          <div class="go">
            ...
            <el-button icon="el-icon-search" @click="handleSearch" type="primary" size="mini"></el-button>
          </div>
          <div class="location">
            <a>三亚</a>
            <a>苏州</a>
        ...
        <div class="main-in-top">
          ...
          <div class="main-in-top-in" >
            <div class="main-in-yi">
              <a href>
                <img src="../../assets/img/wKgBs1c8A8-AQczpAAECxvKp5M438.jpeg" alt />
              </a>
            </div>
            <div class="scenicRmd" v-if="scenicList.length">
              <div >
                  <div class="bigTitle">{{cityInfo.cityName}}</div>
                  <div class="title">热门景点推荐</div>
                  <el-row class="content">
                      <el-col 
                        style="width: 235px; margin-right: 15px;padding-bottom: 15px;" 
                        v-for="(scenicInfo, index) in scenicList" :key="index" >
                          <el-card :body-style="{ padding: '0px' }" @click.native="toDetail(scenicInfo.scenicId)">
                            <img style="width: 235px; height:235px;" :src="'http://101.132.138.185:8080'+scenicInfo.img" class="image">
                          </el-card>
                          <div class="scenicName" @click="toDetail(scenicInfo.scenicId)">{{scenicInfo.scenicName}}</div>
                      </el-col>
                  </el-row>
            ...
            <!-- <scenic-rmd :cityInfo="cityInfo" v-if="cityInfo.cityName"></scenic-rmd> -->
...
import scenicRmd from '@/components/destination/scenicRmd/scenicRmd'
...
export default {
    ...
    methods: {
        ...
        toDetail(scenicId){
          let self = this;
          self.$router.push({ path: "/scenicDetial", query:{id:scenicId}});
        },
        //跳转到热门景区
        toDestination(val) {
            let self = this;
            self.cityInfo = {
                cityCode: self.cityCodeArray[val].cityCode,
                cityName: self.cityCodeArray[val].cityName
            }
            self.isShow = false;
            // document.getElementById('scenicRmd').scrollIntoView(true); //滚动到指定元素位置
        ...
        handleSearch() {
            ...
            self.cityInfo = {
                cityCode: pathvalue.path[1],
                cityName: pathvalue.pathLabels[1]
            }
            fetchList(
                self.listQuery.page,
                self.listQuery.size,
                self.cityInfo.cityCode
            ).then(res => {
                self.scenicList = res.data.records;
            });
            // self.isShow = false;
            document.documentElement.scrollTop = 530 
        },
