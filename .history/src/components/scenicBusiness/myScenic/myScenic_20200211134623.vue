        ...
        <el-card class="content-container">
            <div class="table-container">
                <el-table ref="scenicTable"
                    ...
                    <el-table-column label="酒店名称" prop="scenicName" :show-overflow-tooltip='istooltip' align="center">
                    </el-table-column>
    ...
    methods: {
        ...
        queryData(flag){
            ...
            if(flag === 1){
                ...
                queryArea(self.editScenic.cityCode, 1, 1000).then(response => {
                    let list = response.data.records;
                    const nodes = list.map(x=>({
                        value: x.areaCode,
                        label: x.shortName,
                ...
                queryStreet(self.editScenic.areaCode, 1, 1000).then(response => {
                    let list = response.data.records;
                    const nodes = list.map(x=>({
                        value: x.streetCode,
                        label: x.shortName,
