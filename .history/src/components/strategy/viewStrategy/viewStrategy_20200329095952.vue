 ...
 * @LastEditors: lixiang
 * @LastEditTime: 2020-03-29 09:59:52
 -->
<template>
  <div class="srategy-container">
    <div style="margin: 20px 160px 0 160px;" v-loading="isloading">
      <div class="city">
      ...
      <div class="two">
        作者：
        <span class="userName">{{strategyInfo.username}}</span>
        创建日期：
  ...
  methods: {
    ...
    handleCollect() {
      ...
      collectArticleById(self.strategyId).then(res => {
        self.strategyInfo.collection = res.data.replace(/[^0-9]/gi, "");
        this.$message({
          message: "收藏成功",
          type: "success",
          duration: 1000
