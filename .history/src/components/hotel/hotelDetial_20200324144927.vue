 ...
 * @LastEditors: lixiang
 * @LastEditTime: 2020-03-24 14:49:27
 -->
<template>
    ...
    <el-dialog
      ...
      <el-form 
        ...
        <el-form-item label="订单总价："  >
          <el-input v-model="total" v-show="false"></el-input>
            <span v-if="orderInfo.total" style="font-size: 22px; color: red;">{{orderInfo.total}}¥</span>
        </el-form-item>
      </el-form>
...
export default {
  ...
  computed:{
    // total () {
    //   let total = 0;
    //   console.log(this.roomDaterange)
  ...
  methods: {
    changeDate(){
      let total = 0;
      console.log(this.orderInfo.changeDate)
      let start = new Date(this.orderInfo.changeDate[0]);
      let end = new Date(this.orderInfo.changeDate[1]);
      let day = (end - start)/(24*60*60*1000) + 1;
      console.log(start)
    ...
    handleOrder(row){
      ...
      if(self.roomDaterange.length){
        this.$set(self.orderInfo , "daterange", self.roomDaterange);
      }
