 ...
 * @LastEditors: lixiang
 * @LastEditTime: 2020-03-24 16:02:24
 -->
<template>
    ...
    <el-dialog
      ...
      <el-form 
        ...
        <el-form-item label="订单总价："  >
            <span style="font-size: 22px; color: red;">{{orderInfo.total?orderInfo.total:0}}¥</span>
        </el-form-item>
      </el-form>
...
import { formatDat, generateFun } from '../../utils/date'
...
export default { 
  ...
  methods: {
    ...
    changeNum(){
      ...
      let end = new Date(this.orderInfo.daterange[1]);
      let day = (end - start)/(24*60*60*1000);
      this.orderInfo.total = day*this.orderInfo.sellPrice*this.orderInfo.num;
    },
    //修改时间戳
    changeClick(e){
      let self = this;
    ...
    handleEditConfirm(){
      ...
      for(let i = 0; i<arrlength; i++){
        ids.push(self.orderInfo.hotelId);
        nums.push(self.orderInfo.num);
        types.push(0);
      }
      dates = generateFun(new Date(self.orderInfo.daterange[0]), new Date(self.orderInfo.daterange[1]))
      console.log(dates)
