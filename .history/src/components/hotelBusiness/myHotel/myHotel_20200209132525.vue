 ...
 * @LastEditors  : lixiang
 * @LastEditTime : 2020-02-09 13:25:25
 -->
<template>
...
export default {
            ...
            props: {
                ...
                async lazyLoad (node, resolve) {
                    ...
                    if(level === 0){
                        ...
                        await queryStreet(node.data.value, 1, 1000).then(response => {
                            let list = response.data.records;
                            const nodes = list.map(x=>({
                                value: x.streetCode,
                                label: x.shortName,
    ...
    methods: {
        ...
        handleSearch(index, row){
            ...
            if(row.streetCode){
                self.areaCode = [
                    row.streetCode.slice(0,2)+'0000',
                    row.streetCode.slice(0,4)+'00',
                    row.streetCode.slice(0,6),
                    row.streetCode
                ]
            }
