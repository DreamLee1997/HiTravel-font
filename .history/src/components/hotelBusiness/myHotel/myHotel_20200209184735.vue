 ...
 * @LastEditors  : lixiang
 * @LastEditTime : 2020-02-09 18:47:34
 -->
<template>
    <div class="app-container">
        ...
        <el-form 
            ...
            <el-form-item label="所属街道：" prop="areaCode">
                <!-- <v-distpicker 
                    disabled
                    :province="editHotel.provinceName+'省'" 
                    :city="editHotel.cityName+'市'" 
                    :area="editHotel.areaName+'区'"></v-distpicker> -->
                <el-select 
                    :disabled="isEdit"
...
export default {
    ...
    methods: {
        getOptions(row){
            // await this.getCityOptions(row.cityCode);
            // await this.getAreaOptions(row.areaCode);
            this.getStreetOptions(row.streetCode);
        },
        //查看酒店信息
        ...
        //获取所有市
        getCityOptions(code){
            queryCity(code, 1, 1000).then(response => {
                let list = response.data.records;
                const nodes = list.map(x=>({
                    value: x.cityCode,
                    label: x.shortName,
        ...
        //获取所有区
        async getAreaOptions(code){
            queryArea(code, 1, 1000).then(response => {
                let list = response.data.records;
                const nodes = list.map(x=>({
                    value: x.areaCode,
                    label: x.shortName,
                ...
                this.getCityOptions(row.areaCode);
            });
        },
        //获取所有街道
        async getStreetOptions(code){
            queryStreet(code, 1, 1000).then(response => {
                let list = response.data.records;
                const nodes = list.map(x=>({
                    value: x.streetCode,
                    label: x.shortName,
                ...
                this.streetCodeOptions = nodes;
                this.getAreaOptions(row.areaCode);
            });
        },
        handleCancle(){
            let self = this;
