 ...
 * @LastEditors  : lixiang
 * @LastEditTime : 2020-02-07 15:58:00
 -->
<template>
...
export default {
        ...
        return {
            ...
            props: {
                ...
                async (node, resolve) {
                    const { level } = node;
                    if(level === 0){
                        await queryProvince(1, 1000).then(response => {
                        ...
                        await queryCity(node.data.value, 1, 1000).then(response => {
                            let list = response.data.records;
                            const nodes = list.map(x=>({
                                value: x.cityCode,
                                label: x.shortName,
                        ...
                        await queryArea(node.data.value, 1, 1000).then(response => {
                            let list = response.data.records;
                            const nodes = list.map(x=>({
                                value: x.areaCode,
                                label: x.shortName,
    ...
    methods: {
        ...
        async filterStreetCode(code){
            ...
            await queryStreet(self.areaCode[2], 1, 1000).then(response => {
                let list = response.data.records;
                const nodes = list.map(x=>({
                    value: x.streetCode,
                    label: x.shortName,
