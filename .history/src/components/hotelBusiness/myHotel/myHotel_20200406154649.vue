 ...
 * @LastEditors: lixiang
 * @LastEditTime: 2020-04-06 15:46:49
 -->
<template>
                ...
                <el-table ref="hotelTable"
                    ...
                    <el-table-column label="详细地址" :show-overflow-tooltip='istooltip' align="center">
                        <template slot-scope="scope">
                            ...
                            + scope.row.areaName + '区' + scope.row.streetName}}
                        </template>
                    </el-table-column>
...
import { queryMyHotel, updateHotel, searchHotel, deleteRoom } from '@/api/hotelBusiness'
    ...
    methods: {
        ...
        handleDelete(index, row){
            ...
            this.$confirm('是否确认删除', '提示', {
                ...
                deleteRoom(row.hotelId).then(response => {
                    this.$message({
                        message: '删除成功',
                        type: 'success',
                        duration: 1000
        ...
        handleEditConfirm(){
            ...
            let streetItem = {};
            //获取省label
            provinceItem = self.provinceCodeOptions.find((item)=>{
                return item.value === self.editHotel.provinceCode;//筛选出匹配数据
            });
            //获取市label
            cityItem = self.cityCodeOptions.find((item)=>{
                return item.value === self.editHotel.cityCode;//筛选出匹配数据
            });
            //获取区label
            areaItem = self.areaCodeOptions.find((item)=>{
                return item.value === self.editHotel.areaCode;//筛选出匹配数据
            });
            //获取街道label
            streetItem = self.streetCodeOptions.find((item)=>{
                return item.value === self.editHotel.streetCode;//筛选出匹配数据
            });
            self.editHotel.provinceName = provinceItem.label;
            self.editHotel.cityName = cityItem.label;
            self.editHotel.areaName = areaItem.label;
            self.editHotel.streetName = streetItem.label;
