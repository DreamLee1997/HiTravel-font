 ...
 * @LastEditors  : lixiang
 * @LastEditTime : 2020-02-10 12:41:41
 */
import { login, logout, getInfo } from '@/api/login'
import { getToken, setToken, removeToken, getUserName, setUserName, getRoles, setRoles } from '@/utils/auth'
    ...
    Login({ commit }, {userInfo, vue}) {
      ...
      return new Promise((resolve, reject) => {
        login(userInfo.username, userInfo.password, userInfo.userType).then(response => {
          ...
          sessionStorage['token']=JSON.stringify(tokenStr)
          // setTimeout(() => {
          ...
          if(data.type){
            if(data.type === "1"){
              ...
              //景区商户
              vue.$router.push({path:'/scenicBusiness'});
            }
          }
          // Message('登陆成功');
          // vue.$router.push({path:'/index'});
